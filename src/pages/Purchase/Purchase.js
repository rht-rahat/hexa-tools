...
const Purchase = () => {
  ...
  const {
    ...
    fetch(`https://hexa-tools-server.vercel.app/product/${_id}`).then((res) => {
      return res.json();
    })
  );
  ...
  const handleOrder = (data) => {
    ...
    fetch("https://hexa-tools-server.vercel.app/order", {
      method: "POST",
      headers: {
      ...
      .then((data) => {
        if (data.insertedId) {
          ...
          fetch(
            `https://hexa-tools-server.vercel.app/product-available/${product._id}`,
            {
              method: "PATCH",
              body: JSON.stringify({
